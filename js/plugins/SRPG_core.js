 ...
 * @param textSrpgRange
 * @parent SRPGTerm
 * @desc A term for range of an attack. It is displayed in the SRPG battle prediction window.
 * @default Range
 * 
 ...
 * 	<srpgReactionSkill:X>
 * 		# Use skill with ID set by X in reaction (assumes a passive skill).
 ...
 *  - 戦闘開始前、戦闘中、ユニット行動後の処理、ユニットイベントで
 *    参照することが出来ます。
 *  - イベントIDを指定するスクリプト/プラグインコマンドでは、
 *    0で行動中のイベントID、-1で対象のイベントIDを参照できます。
 * 
 *   this.activeEventId();
        ...
        if (reaction && reaction.item()) {
            ...
            if (reaction.isForUser()) {
                if (reaction.item().meta.srpgAllFriendReaction) {
                    for (var i = 0; i < targetEventsArray.length; i++) {
...
//====================================================================
    ...
    const _SRPG_Game_Unit_onBattleEnd = Game_Unit.prototype.onBattleEnd;
...
//====================================================================
    ...
    Game_Interpreter.prototype.command322 = function() {
        if ($gameSystem.isSRPGMode()) {
            ...
            return _SRPG_Game_Interpreter_command322.call(this);
        }
    };
...
//====================================================================
    ...
    Window_Base.prototype.drawActorLevel = function(actor, x, y, width) {
        width = width || 120;
        this.drawSrpgExpRate(actor, x, y, width);
        this.changeTextColor(this.systemColor());
        this.drawText(TextManager.levelA, x, y, 48);
        this.resetTextColor();
        this.drawText(actor.level, x + 54, y, width - 54, 'right');
    };
...
//====================================================================
    ...
    Window_SrpgStatus.prototype.drawBasicInfoActor = function(x, y) {
        ...
        this.drawActorHp(battler, x, y + lineHeight * 2);
        if ($dataSystem.optDisplayTp) {
            this.drawActorMp(battler, x, y + lineHeight * 3, 90);
            this.drawActorTp(battler, x + 96, y + lineHeight * 3, 90);
        } else {
            this.drawActorMp(battler, x, y + lineHeight * 3);
        }
    };
...
//====================================================================
    ...
    Window_SrpgBattleResult.prototype.initialize = function(battler) {
		...
		var y = (Graphics.boxHeight - height) / 4;
    ...
    Window_SrpgBattleResult.prototype.drawGainItem = function(x, y) {
        ...
        const width = this.contentsWidth() / 2 - padding - this.textWidth('XXXX');
        ...
        for (let i = 0; i < searched.length; i++) {
            ...
            let posY = y + this.lineHeight() * Math.floor(i / 2);
...
//====================================================================
    ...
    // 初期化
    //----------------------------------------------------------------
    const _SRPG_BattleManager_initMembers = BattleManager.initMembers;
    ...
    // ステータスウィンドウのリフレッシュ
    const _SRPG_BattleManager_refreshStatus = BattleManager.refreshStatus;
...
//====================================================================
    ...
    const _SRPG_Scene_MenuBase_previousActor = Scene_MenuBase.prototype.previousActor;
...
//====================================================================
	...
	Scene_Map.prototype.processMapBattle = function(userArray, targetArray) {
        ...
        user.setUsedSkill(action.item().id);
        // queue the action
        // targetEventsにはgame_Eventのリストが入っている。
        // action_targetListにはgame_actor, game_enemyのリストが入っている
		...
		switch (data.phase) {
			...
			case 'start':
				...
				if (!$gameTemp.isFirstAction || $gameTemp.isFirstAction(action)) {
					...
					if (action.item().castAnimation && action.isSkill() && !action.isAttack() && !action.isGuard()) {
						userEvent.requestAnimation(action.item().castAnimation);
						castAnim = true;
					}
					// target animation
			...
			case 'reflect':
				...
				if (target.reflectAnimationId) {
					targetEvent.requestAnimation(target.reflectAnimationId());
				}
				data.target = user;
				data.phase = 'animation';
				this._srpgSkillList.unshift(data);
			...
			case 'animation':
				...
				if (animationData && 
                    ...
                    targetEvent.requestAnimation(animation);
